...
def download_frida(device: Device, version: str = None):
    if version:
        latest_release = requests.get("https://api.github.com/repos/frida/frida/releases/tags/%s" % version).json()
    else:
...
def update_server(version=None):
    if version:
        eprint("⚡️ Updating frida-server with version %s" % version)
    else:
        eprint("⚡️ Updating frida-server with latest version")
    devices = get_devices()
    for device in devices:
        eprint("📲 Device: {} ({})".format(get_device_model(device), device.get_serial_no()))
